...
import {
    ...
    Timer, History, ChevronUp, ChevronDown, Copy
} from 'lucide-react';
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC]">
            ...
            <AnimatePresence>
                ...
                {isEditModalOpen && <OrderDetailsDrawer isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} onSave={updateProgress} order={selectedOrder} />}
            </AnimatePresence>
        </div>
    );
}
    ...
    return (
        <div className="p-5 space-y-5 bg-white sm:p-6">
            ...
            <div className="flex gap-3">
                <button
                    onClick={() => {
                        navigator.clipboard.writeText(order.targetLink);
                    }}
                    className="flex-1 py-2.5 bg-slate-50 text-slate-600 hover:text-slate-900 hover:bg-slate-100 rounded-xl text-[10px] font-black uppercase tracking-wider flex items-center justify-center gap-2 transition-colors cursor-pointer"
                >
...
function OrderRow({ order, index, urgency, onUpdate, onMarkDone, onEdit }: any) {
    ...
    return (
        <tr className="group hover:bg-slate-50/70 transition-colors">
            ...
            <td className="px-8 py-5">
                <div className="flex items-center gap-2">
                    <a href={order.targetLink} target="_blank" className="flex items-center gap-1 text-slate-500 hover:text-slate-900 transition-colors font-bold text-xs group/link">
                        <span className="max-w-[150px] truncate">{order.targetLink.replace('https://', '').replace('www.', '')}</span>
